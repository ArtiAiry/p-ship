    ...
    <div class="card fade-out">
        <div class="card-body">
            <h1><?= Html::encode($this->title) ?>
                <button value="<?= Url::to(['/company/create']) ?>" class="btn btn-outline-primary" id="modalButton1"
                        data-toggle="tooltip" data-placement="bottom"
                        title="<?= Module::t('company', 'Create a Company') ?>">
                        ...
                        <?php if ($company->isRemoved() == 1): ?>
                            ...
                            <td>
                                <button value="<?= Url::toRoute(['/company/view', 'id' => $company->id]); ?>"
                                        data-toggle="tooltip" title="<?= Module::t('company', 'info') ?>"
                                        aria-label="Info"
                                        class="btn btn-outline-dark btn-rounded btn-xs view-modal-click"><?= Module::t('company',
                                        'info') ?></button>
                                <a href="<?= Url::toRoute(['/product/company', 'name' => $company->name]); ?>"
                                   data-toggle="tooltip" title="<?= Module::t('company', 'go to products') ?>"
                                   aria-label="Go to products"
                                   class="btn btn-outline-dark btn-rounded btn-xs update-modal-click"><?= Module::t('company',
                                        'go to products') ?></a>
                            </td>
                            <?php if ($profile->user->getRole() == 'admin'): ?>
                                <td><?= Yii::$app->formatter->asDatetime($company->created_at) ?></label></td>
                                <td><?= Yii::$app->formatter->asDatetime($company->updated_at) ?></td>
                                <td>
                                    <button value="<?= Url::toRoute(['/company/view', 'id' => $company->id]); ?>"
                                            data-toggle="tooltip" title="<?= Module::t('company', 'View') ?>"
                                            aria-label="View"
                                            class="btn btn-outline-dark btn-rounded btn-xs view-modal-click"><span
                                                class="fa fa-eye"></span></button>
                                    <a href="<?= Url::toRoute(['/company/update', 'id' => $company->id]); ?>"
                                       data-toggle="tooltip" title="<?= Module::t('company', 'Update') ?>"
                                       aria-label="Update"
                                       class="btn btn-outline-dark btn-rounded btn-xs update-modal-click"><span
                                                class="fa fa-pencil"></span></a>
                                    <a href="<?= Url::toRoute(['/company/remove', 'id' => $company->id]); ?>"
                                       data-toggle="tooltip" title="<?= Module::t('company', 'Delete') ?>"
                                       aria-label="Delete" data-confirm="<?= Module::t('company',
...
<?php
Modal::begin([
    'header' => '<h4>' . Module::t('company', 'Info') . '</h4>',
    'id' => 'view-modal',
    'size' => 'modal-md',
]);
