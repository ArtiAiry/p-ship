    ...
    <div class="card-body">
        <h1><?= Html::encode($this->title) ?>
            <?php if ($profile->user->getRole() == 'admin'): ?>
                <button value="<?= Url::to(['/leads/create']) ?>" class="btn btn-outline-primary" id="modalButton1"
                        data-toggle="tooltip" data-placement="bottom"
                        title="<?= Module::t('leads', 'Create a Lead') ?>">
        ...
        <table id="leads-table" class="table table-hover table-bordered dt-responsive nowrap" style="width:100%">
            <thead>
            <tr>
                <td><?= Module::t('leads', 'Created At') ?> <i class="mdi mdi-help-circle-outline" data-toggle="tooltip"
                                                               data-placement="top"
                                                               title="Отображение, сортировка и вывод дат происходит по дд/мм/гггг."></i>
                </td>
                <?php if ($profile->user->getRole() == 'admin'): ?>
                        ...
                        <?php if ($profile->user->getRole() == 'admin'): ?>
                            ...
                            <td>
                                <button value="<?= Url::toRoute(['/leads/view', 'id' => $lead->id]); ?>"
                                        data-toggle="tooltip" title="<?= Module::t('leads', 'View') ?>"
                                        aria-label="View"
                                        class="btn btn-outline-dark btn-rounded btn-xs view-modal-click"><span
                                            class="fa fa-eye"></span></button>
                                <a href="<?= Url::toRoute(['/leads/update', 'id' => $lead->id]); ?>"
                                   data-toggle="tooltip" title="<?= Module::t('leads', 'Update') ?>" aria-label="Update"
                                   class="btn btn-outline-dark btn-rounded btn-xs update-modal-click"><span
                                            class="fa fa-pencil"></span></a>
                                <a href="<?= Url::toRoute(['/leads/remove', 'id' => $lead->id]); ?>"
                                   data-toggle="tooltip" title="<?= Module::t('leads', 'Delete') ?>" aria-label="Delete"
                                   data-confirm="<?= Module::t('leads',
