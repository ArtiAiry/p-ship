                ...
                <table id="min-table" class="table table-hover table-bordered dt-responsive nowrap" style="width:100%">
                    ...
                    <?php foreach ($statuses as $status): ?>
                        <tr>
                            ...
                            <td><?= $status->name ?></td>
                            <td>
                                <button value="<?= Url::toRoute(['/leads/status/view', 'id' => $status->id]); ?>"
                                        data-toggle="tooltip" title="<?= Module::t('status', 'View') ?>"
                                        aria-label="View"
                                        class="btn btn-outline-dark btn-rounded btn-xs view-modal-click"><span
                                            class="fa fa-eye"></span></button>
                                <a href="<?= Url::toRoute(['/leads/status/update', 'id' => $status->id]); ?>"
                                   data-toggle="tooltip" title="<?= Module::t('status', 'Update') ?>"
                                   aria-label="Update"
                                   class="btn btn-outline-dark btn-rounded btn-xs update-modal-click"><span
                                            class="fa fa-pencil"></span></a>
                                <a href="<?= Url::toRoute(['/leads/status/remove', 'id' => $status->id]); ?>"
                                   data-toggle="tooltip" title="<?= Module::t('status', 'Delete') ?>"
                                   aria-label="Delete"
                                   data-confirm="<?= Module::t('status',
